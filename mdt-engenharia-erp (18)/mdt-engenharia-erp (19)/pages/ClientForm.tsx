...
export const ClientForm: React.FC = () => {
  ...
  const saveClient = async () => {
    ...
    try {
      // Create a clean payload with only necessary fields and proper null handling
      const clientPayload = {
        name: client.name,
  ...
  const openWorkflowModal = async (item: ServiceItem) => {
      ...
      // Fetch Comments
      const { data: comments } = await supabase.from('service_item_comments').select('*').eq('itemId', item.id).order('date', { ascending: true });
      setWorkflowComments(comments || []);

      // Fetch Attachments
      const { data: attachments } = await supabase.from('service_item_attachments').select('*').eq('itemId', item.id);
  ...
  const handleWorkflowChange = (field: keyof ServiceItem, value: any) => {
      ...
      setActiveItemForWorkflow(prev => prev ? ({ ...prev, [field]: value }) : null);
  };
  ...
  const addWorkflowComment = async (e: React.FormEvent) => {
      ...
      if (!error) {
          const { data } = await supabase.from('service_item_comments').select('*').eq('itemId', activeItemForWorkflow.id).order('date', { ascending: true });
  ...
  const addWorkflowAttachment = async () => {
      ...
      if (url) {
          await supabase.from('service_item_attachments').insert([{
              itemId: activeItemForWorkflow.id,
          ...
          const { data } = await supabase.from('service_item_attachments').select('*').eq('itemId', activeItemForWorkflow.id);
            ...
            <div className="fixed inset-0 z-50 bg-gray-900 bg-opacity-70 flex items-center justify-center p-4 backdrop-blur-sm">
                <div className="bg-white rounded-xl shadow-2xl w-full max-w-4xl overflow-hidden flex flex-col max-h-[90vh]">
                        ...
                        <div className="md:w-5/12 p-0 flex flex-col bg-white h-[500px] md:h-auto">
                            ...
                            <div className="flex-1 overflow-y-auto p-4 space-y-3 bg-gray-50" style={{maxHeight: '300px'}}>
                                ...
                                {workflowComments.map(c => (
                                    <div key={c.id} className="bg-white p-2.5 rounded shadow-sm border border-gray-100 text-sm">
                            ...
                            <div className="p-3 border-t border-gray-200 bg-white">
                                <form onSubmit={addWorkflowComment} className="flex gap-2 mb-4">
                                    <input 
                                    ...
                                    />
                                    <button type="submit" className="bg-purple-600 text-white p-1.5 rounded-full hover:bg-purple-700"><Send className="w-4 h-4"/></button>
                                </form>
                                ...
                                <div className="border-t pt-3">
                                    ...
                                    <div className="flex gap-2 mb-2">
                                        ...
                                        />
                                        <button onClick={addWorkflowAttachment} disabled={uploadingWorkflowDoc} className="text-xs bg-gray-200 hover:bg-gray-300 px-2 py-1 rounded">
                    ...
                    <div className="bg-gray-50 p-4 border-t border-gray-200 flex justify-end">
                        <button onClick={saveWorkflow} className="bg-purple-600 text-white px-6 py-2 rounded shadow hover:bg-purple-700 font-medium text-sm flex items-center">
